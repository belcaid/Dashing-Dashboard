...
SCHEDULER.every '30m', :first_in => 0 do
 ...
 # Déclaration des array
  typeappareil = Array.new
  visites = Array.new
  results = Array.new
  
  # Éxecution de la requête
    visitespartypeappareil_List = client.execute(:api_method => analytics.data.ga.get, :parameters => { 
    'ids' => "ga:" + profileID, 
 ...
 typeappareil = visitespartypeappareil_List.data.rows.transpose[0]
 visites = visitespartypeappareil_List.data.rows.transpose[1]
 
 # Affectation des données
 compteur= 0
 typeappareil.each do |row|
 results.push({label: typeappareil[compteur], value: visites[compteur]})
 compteur += 1
 end
 
 
  # Mise à jour du tableau de bord
  send_event('visitespartypeappareilm_list',   { items: results })
end
